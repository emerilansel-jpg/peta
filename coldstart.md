...
> - Aggregate-only staging inventory: 2 pending/paid payouts, 1 active task, 1 open order, 0 approved assignments. No historical reward backfill needed for approved rows, but financial-policy replacement must preserve existing obligations/contracts. Safe path remains targeted compatibility adapters and additive columns/tables plus reviewed trigger/constraint replacements; no destructive operation is demonstrated necessary. This prerequisite work is unfinished, not a user-decision blocker. Do not deploy production before it and staging lifecycle gate pass.
>
> Previous live-schema reconciliation (2026-09-10): **STAGING REACHABLE; RELEASE GATE FAILS LEGACY CLAIM REGRESSION. No remote schema/data mutation, commit, push, or deployment.**
> - Fresh CLI auth only, no `SUPABASE_ACCESS_TOKEN` override. Isolated staging link succeeded; READ ONLY schema inventory succeeded after project initially reported RESTORING. Existing workspace production link preserved.
> - Actual staging: 15 recorded migrations, latest `20260715173313`; production: 71, latest `20260727`. Staging lacks `reddit_order_comment_drafts`, `straight_settings`, `reddit_army_profiles`, `payout_allocations`; assignment columns `balance_credited_at`, `expires_at`, `proof_media`, `user_note`; approval/ownership/slot-sync triggers. Staging checkout has 9 parameters versus production's 13. Three release files alone cannot be safely applied there. Scoped prerequisite reconciliation remains necessary; no blanket push.
> - **Confirmed default-OFF regression:** production `claim_task_assignment` inserts assignment then assigns its reserved forum draft via UPDATE. Proposed `guard_assignment_authorization` rejects that legitimate SECURITY DEFINER update with `assignment_draft_immutable`, even with empty allowlist. Added real checkout-to-claim regression to `peta/scripts/test-workflow-replay.py`; local run exits 3 with that exact error. Earlier checkout tests pass, but were insufficient. Initial added fixture omitted explicit drafts and failed its assertion; corrected fixture passes `p_comment_drafts` and reproduces the actual guard error.
> - **Additional default-OFF incompatibility:** live `cancel_expired_assignments` rejects overdue assignments and writes admin fields without a user JWT; new guard rejects this path. Production cron job `cancel-expired-assignments` is active (read-only verified). Needs narrowly authorized internal path, not blanket SECURITY DEFINER/service bypass.
> - Private live function/column/trigger inventories stored outside git at `G:/SF Project/peta-backups/release-reconcile-20260910/`. Never commit these files; function bodies may contain sensitive configuration. Local replay still has 40 managed-extension restore errors, no external effects, not full Supabase parity.
> - Staging gate NOT passed. Production backup/deploy intentionally not started. Pilot preview/revise UI and two-track onboarding remain deferred; default-off safety not yet established. Next: fix legacy claim/expiry compatibility without weakening direct-write guards, add real lifecycle regressions, reconcile scoped staging prerequisites, then staging transaction tests before any production release.
>
> Previous fresh-login continuation (2026-09-10): **AUTH RESTORED; RELEASE BLOCKED BY PAUSED STAGING. No commit, push, deployment, or remote DB mutation.**
> - Fresh locally configured CLI login lists both required projects; no `SUPABASE_ACCESS_TOKEN` environment override, no prior supplied token used. Temporary local links only; original workspace link preserved.
> - Staging `duxzxizedtvnopfihllz`: CLI link returns `project is paused`. Admin must resume at https://supabase.com/dashboard/project/duxzxizedtvnopfihllz before migration inventory and staging lifecycle verification can run. No unpause attempted.
> - Production `yorlsgzsawchpeeazcvi`: temporary link and explicit `BEGIN READ ONLY` inventory succeeded. 71 recorded migration versions, latest `20260727`; local 128 SQL files / 90 distinct prefixes, 55 files without recorded versions, 13 duplicate local version prefixes. History is not reliable evidence of actual schema absence: historical direct SQL deployments need reconciliation. Do NOT blanket `db push` or repair history blindly.
> - Production release tables `contributor_pilot_users`, `straight_order_screenings`, `legacy_paid_vote_orders` are absent. All three `20260910` release versions unrecorded. Exact proposed snapshot predicate covers 2/2 currently pending/processing paid-vote orders (aggregate-only read; no customer rows printed).
> - An initial legacy coverage query used nonexistent `reference_id` and failed read-only; corrected query uses the migration's `metadata->>'reddit_upvote_order_id'` predicate and succeeded. No failed query counted as a pass.
> - Inspected isolated scripts: replay recreates ONLY local `workflow_replay` at loopback port 55439, restores schema only, inserts synthetic users, stubs outbound HTTP. Focused local fixture is not full Supabase parity. No tests rerun this continuation; prior limitations below remain. Payments unaffected by this continuation's read-only operations, but release payment compatibility/full claim-to-payout lifecycle remain UNVERIFIED.
> - Hosting config `peta/wrangler.toml`: Cloudflare Pages `peta`, output `./dist`; existing handoff identifies separate `straight` project. No hosting mutation/access claim. Pilot UX remains incomplete; no pilot shipment claim.
> - Next: resume staging, reconcile actual schema against the three reviewed release migrations, run isolated synthetic staging lifecycle with outbound notifications/payment calls disabled, then evaluate production release. No production frontend deploy before compatible verified DB migrations.
>
> Previous continuation (2026-09-10): **RELEASE BLOCKED BY SUPABASE ACCESS. No commit, push, deployment, or remote DB mutation.**
> - Read actual diff/status first: branch `main`, extensive pre-existing untracked artifacts; nothing staged. Existing CLI authentication lists neither required project. Supplied task token returned HTTP 403 on read-only project metadata requests for BOTH staging `duxzxizedtvnopfihllz` and production `yorlsgzsawchpeeazcvi`. Need project-authorized credentials before staging-first release; migration inventory could not be fetched. Temporary token file outside git removed.
> - PASS rerun: `scripts/test-workflow-replay.py` restored backup SCHEMA ONLY into isolated `workflow_replay` on PostgreSQL 17 loopback port 55439, synthetic users only, applied exactly the three `20260910*.sql` release migrations. This is release-delta replay on production schema, NOT all historical migrations or full Supabase parity.
> - 40 restore errors: unavailable pg_cron, pg_net, supabase_vault and dependent objects/grants; six event-trigger ownership errors. HTTP functions stubbed locally, no outbound notifications; cron/vault/external payment-provider effects unverified.
> - PASS real SQL checkout checks: default-off allowlist, paid legacy order task recovery with legacy eligibility, new paid-vote rejection without debit/ledger change, legacy obligation mutation rejection, screening required/single-use, pending paused pilot task, non-Reddit legacy checkout, successful checkout debit matches order costs, failed second bulk item rolls back first order/task/balance/ledger. External payment capture/top-up, full claim-to-payout lifecycle, historical data reconciliation still unverified.
> - Existing interrupted work already contained `legacy_paid_vote_orders` snapshot and fn_ensure legacy exemption; no blanket legacy upvote exception remains for snapshotted paid obligations. No refunds/deletions introduced. Snapshot coverage against live historical paid orders still needs staging/prod read-only assessment.
> - Fixed isolated fixture rerun after replay (existing cluster roles; missing legacy snapshot table). PASS `test-workflow-local.mjs`, offline screening/contributor scripts, `npm run build` (bundle `main-BmBNSJMd.js`; chunk-size/dynamic-import warnings).
> - No browser/live-route/version verification. Screening preview/revise UI and two-track onboarding remain incomplete, pilot must remain inaccessible/default OFF. Do not describe pilot as shipped. No production frontend deploy without compatible verified DB migrations.
>
> Previous local session (2026-09-10): **LOCAL FIXES ONLY — RELEASE BLOCKED. No commit, push, deployment, or remote DB mutation in that session.**
> - Supplied backup reference: `G:/SF Project/peta-backups/supabase-20260910T081920Z/backup.zip` (previously verified 71 MB; not re-inspected here).
> - Unreleased migrations `20260910100000_straight_screening.sql` / `20260910110000_contributor_workflow.sql` edited; new `20260910120000_workflow_authorization.sql` adds DB write guards and queue RPC contract. Apply together only after full staging verification.
> - Empty server-managed `contributor_pilot_users` allowlist defaults OFF. Reddit screening requires allowlisted buyer; pilot assignments require allowlisted contributor. Existing legacy assignments remain legacy. Non-Reddit checkout skips screening.
> - Owner writes cannot approve rewards or alter server fields. First-proof timestamp/deadline server-managed and immutable on retry; NULL visibility/deadline fail closed; unknown visibility cannot auto-reject. Task activation and assignment insertion guard eligibility.
> - TaskDetail now propagates submission RPC errors (no direct-write fallback), sends user_note and image proof. Queue RPC/mapping includes workflow, visibility, deadlines, proof URLs. Corrected invalid forum brief helper signature and nullable platform flags found during real SQL execution.
> - PASS: `npm run build`; existing offline screening/contributor scripts. Those offline scripts are NOT database verification.
> - PASS: real portable PostgreSQL 17.6 at loopback port 55439, `scripts/test-workflow-local.mjs`: production guard/RPC/ledger functions against minimal fixture schema, transactional rollback. Owner/nonowner attacks, direct approval rollback, workflow edits, NULL deadline/visibility, unknown/early rejection, rejected retry immutability, legacy/pilot ledger counts, activation/claim isolation, queue contract, non-Reddit task creation tested.
> - LIMIT: not a full historical Supabase migration replay; screening checkout/payment transaction migration not exercised against real DB; existing quota/claim/RLS trigger combination and external side effects not fully reproduced. Live staging/prod schema not inspected. DB safety for release remains UNPROVEN.
> - BLOCKERS: explicit screening preview/revise UI absent (still hidden automatic screening); genuine two-track onboarding absent; no browser/screenshots verification this session; paid-Reddit-vote shutdown remains GLOBAL, not pilot-scoped. Existing pending vote fulfillment / legacy payment implications need explicit release decision. No new paid voting enabled.
> - Next: resolve shutdown/payment policy, complete missing UX, replay all migrations in isolated Supabase then staging, verify admin screenshots and complete claim/checkout/payout lifecycle. Do not follow older direct-to-production instructions below.
>
> Previous (2026-08-05): **Fitur baru + bug fix — semua LIVE di prod.**
